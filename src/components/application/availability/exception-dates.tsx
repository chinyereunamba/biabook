...
import { isValidTimeFormat, isEndTimeAfterStartTime, isValidDateFormat } from "@/server/repositories/utils/availability-validation";
...
export function ExceptionDates({
  ...
  const handleInputChange = (field: keyof Omit<ExceptionDate, 'id'>, value: string | boolean) => {
  ...
  const validateException = (): boolean => {
    ...
    if (newException.isAvailable) {
      if (newException.startTime && !isValidTimeFormat(newException.startTime)) {
    ...
    const existingException = exceptions.find(e => e.date === newException.date);
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="exception-available">
                  {newException.isAvailable ? "Available (special hours)" : "Unavailable (closed)"}
                </Label>
              </div>
              ...
              {newException.isAvailable && (
                <>
                  ...
                  {errors.timeRange && <span className="text-xs text-red-500">{errors.timeRange}</span>}
                </>
              )}
      ...
      <CardContent>
        {exceptions.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            No exception dates set. Add special hours or closures for holidays and other events.
          </div>
        ) : (
          <div className="space-y-4">
            {exceptions.sort((a, b) => a.date.localeCompare(b.date)).map((exception) => (
              <div key={exception.id || exception.date} className="flex items-center justify-between rounded-lg border p-4">
                <div className="flex flex-col">
                  <span className="font-medium">{formatDate(exception.date)}</span>
                  <span className={`text-sm ${exception.isAvailable ? 'text-green-600' : 'text-red-600'}`}>
