...
export type UpdateAvailabilityExceptionInput = Partial<
  Omit<CreateAvailabilityExceptionInput, "businessId" | "date">
> & {
  id: string;
...
export class AvailabilityExceptionRepository {
  ...
  async create(
    ...
    if (
      input.startTime &&
      input.endTime &&
      !isEndTimeAfterStartTime(input.startTime, input.endTime)
    ) {
      throw new Error("End time must be after start time");
    }

    // Check for existing exception on the same date
    const existingException = await this.findByBusinessIdAndDate(
      input.businessId,
      input.date,
    );
  ...
  async update(
    ...
    const existingException = await this.findByIdAndBusinessId(
      input.id,
      input.businessId,
    );
    if (!existingException) {
    ...
    const endTime =
      input.endTime !== undefined ? input.endTime : existingException.endTime;
    ...
    if (input.reason !== undefined)
      updateData.reason = input.reason?.trim() ?? null;
    ...
    const [updatedException] = await db
      ...
      .where(
        and(
          ...
          eq(availabilityExceptions.businessId, input.businessId),
        ),
      )
      .returning();
...
export const availabilityExceptionRepository =
