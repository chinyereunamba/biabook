...
export type UpdateWeeklyAvailabilityInput = Partial<
  Omit<CreateWeeklyAvailabilityInput, "businessId" | "dayOfWeek">
> & {
  id: string;
  businessId: string; // Required for authorization
};

export type UpsertWeeklyAvailabilityInput = Omit<
...
export class WeeklyAvailabilityRepository {
  ...
  async create(
    ...
    const existingAvailability = await this.findByBusinessIdAndDay(
      input.businessId,
      input.dayOfWeek,
    );
    if (existingAvailability.length > 0) {
      for (const existing of existingAvailability) {
        if (
          ...
          throw new Error(
            "This time range overlaps with existing availability for this day",
          );
        }
      }
    }
   ...
   */
  async update(
    input: UpdateWeeklyAvailabilityInput,
  ): Promise<WeeklyAvailability> {
    if (!input.id?.trim()) {
    ...
    const existingAvailability = await this.findByIdAndBusinessId(
      input.id,
      input.businessId,
    );
    ...
    for (const other of otherAvailability) {
      if (
        isTimeOverlapping(startTime, endTime, other.startTime, other.endTime)
      ) {
        throw new Error(
          "This time range overlaps with existing availability for this day",
        );
    ...
    const [updatedAvailability] = await db
      ...
      .where(
        and(
          eq(weeklyAvailability.id, input.id),
          eq(weeklyAvailability.businessId, input.businessId),
        ),
      )
      .returning();
   ...
   */
  async upsert(
    input: UpsertWeeklyAvailabilityInput,
  ): Promise<WeeklyAvailability> {
  ...
  async bulkSet(
    businessId: string,
    availabilityEntries: Omit<CreateWeeklyAvailabilityInput, "businessId">[],
  ): Promise<WeeklyAvailability[]> {
    if (!businessId?.trim()) {
      throw new Error("Business ID is required");
    }
    ...
    const entriesByDay = availabilityEntries.reduce<
      Record<number, typeof availabilityEntries>
    >((acc, entry) => {
