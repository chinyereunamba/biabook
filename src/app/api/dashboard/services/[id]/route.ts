...
import { serviceRepository, type UpdateServiceInput } from "@/server/repositories/service-repository";
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Get the service
    const service = await serviceRepository.findByIdAndBusinessId(params.id, business.id);
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (body.bufferTime !== undefined && (typeof body.bufferTime !== "number" || body.bufferTime < 0)) {
      return NextResponse.json(
    ...
    if (body.description !== undefined) updateInput.description = body.description?.trim();
    if (body.duration !== undefined) updateInput.duration = body.duration;
    if (body.price !== undefined) updateInput.price = body.price;
    if (body.category !== undefined) updateInput.category = body.category?.trim();
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    let success: boolean;
    if (hardDelete) {
      success = await serviceRepository.hardDelete(params.id, business.id);
    } else {
      success = await serviceRepository.softDelete(params.id, business.id);
    }
    ...
    return NextResponse.json({ 
      message: hardDelete ? "Service permanently deleted" : "Service deactivated",
      deleted: hardDelete 
    });
  } catch (error) {
